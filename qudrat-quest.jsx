...
/* ═══════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════ */
/* 🔌 جسر التخزين: داخل Claude يستخدم window.storage، وعلى الويب localStorage — مع ترحيل تلقائي */
...
const QQ = {
  ...
  manifest() {
    ...
    return { mcq: this._core.mcq.length, num: this._core.num.length, rich: this._rich.length,
...
const bankPick = ({ topics, n, diffs = [1, 2, 3], g = null, exclude = [] }) => {
  const pool = BANK.filter(x => topics.includes(x.topic) && diffs.includes(x.diff) && !exclude.includes(x.id));
  if (!g) return [...pool].sort(() => Math.random() - 0.5).slice(0, n);
  const byT = {}; topics.forEach(t => byT[t] = pool.filter(x => x.topic === t).sort(() => Math.random() - 0.5));
  const out = [];
  while (out.length < n) {
    const avail = topics.filter(t => byT[t].length);
...
const bankSimilar = (q) => {
  const c = BANK.filter(x => x.topic === q.topic && x.id !== q.id && x.type === "mcq" && x.diff <= q.diff);
...
function buildChallenges(secs, count, g, isBoss) {
  ...
  if (quantIn.length) {
    ...
    const nums = NUMQ.filter(n => n.sec === target).sort(() => Math.random() - 0.5).slice(0, 2)
      .concat(NUMQ.filter(n => quantIn.includes(n.sec) && n.sec !== target).sort(() => Math.random() - 0.5))
      .slice(0, 2).map(n => ({ kind: "num", ...n }));
  ...
  if (hasVerbal && (weightOf(g, "vocab") > 45 || Math.random() < 0.6)) {
    ...
    const m = learned.length >= 4 && Math.random() < 0.7
      ? { pairs: [...learned].sort(() => Math.random() - 0.5).slice(0, 4).map(w => [w.w, w.ar]) }
...
const secAcc = (g, s) => { const v = secStat(g, s); return v.a ? Math.round((v.c / v.a) * 100) : null; };
const secAvgT = (g, s) => { const v = secStat(g, s); return v.a ? Math.round((v.t || 0) / v.a) : null; };
...
function Academy({ g, theme, onExit, onPlace, onFinishUnit, onSimDone, onReview, onOpenUnit, startReview }) {
  ...
  if (sub === "place") return <Placement theme={theme} onDone={(rec) => { onPlace(rec); setSub(null); }} onBack={() => setSub(null)} />;
    ...
    <div style={{ animation: "drop .3s ease" }}>
      ...
      <div className="card" style={{ textAlign: "center", background: "#17251F", color: "#fff", border: "none" }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 10 }}>
          ...
          <button className="btn gold" style={{ padding: "8px 14px", fontSize: 12.5 }} onClick={() => { play("click"); setSub("ask"); }}>💬 اسأل المعلم</button>
        </div>
      </div>
...
function Placement({ theme, onDone, onBack }) {
  ...
  return (
    <div style={{ animation: "drop .3s ease" }}>
      ...
      <div className="card">
        <div style={{ fontSize: 12, fontWeight: 900, color: "#C89235", marginBottom: 8 }}>🧪 تحديد المستوى • {i + 1}/5 — لا توجد إجابة "غلط"، فقط خريطة</div>
        ...
        {q.options.map((o, idx) => {
          ...
          return <button key={idx} className="opt" style={st} onClick={() => {
            ...
            const ns = score + (idx === q.a ? 1 : 0);
            setTimeout(() => { setScore(ns); setPicked(null); if (i + 1 >= PLACEMENT.length) setEnd(true); else setI(i + 1); }, 650);
...
function LessonPlayer({ g, theme, unit, onDone, onBack, test, onOpen }) {
  ...
  const need = unit.need || Math.min(3, unit.drills?.length || 0);
  const drill = unit.drills?.[di % (unit.drills?.length || 1)];
  ...
  if (stage === "pass") return (
    <div className="card" style={{ textAlign: "center", padding: 26, animation: "pop .45s ease", position: "relative", overflow: "hidden" }}>
      ...
      <button className="btn gold" style={{ width: "100%", marginTop: 14, padding: 12 }} onClick={onDone}>{review ? "رجوع" : "استلم مكافأتك ←"}</button>
    </div>
  );
    ...
    <div style={{ animation: "drop .3s ease" }}>
      ...
      {stage === "cards" && <>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button className="btn" style={{ flex: 2, padding: 12 }} onClick={() => { play("click"); ci + 1 < unit.cards.length ? setCi(ci + 1) : setStage(unit.drills ? "drill" : "pass"); }}>
            {ci + 1 < unit.cards.length ? "فهمت — التالي ←" : unit.drills ? "✏️ طبّق اللي تعلمته" : "إنهاء"}
          </button>
        </div>
      </>}
...
function AICheck({ theme, data, onBonus }) {
  const [picked, setPicked] = useState(null);
  if (!data?.check?.options) return null;
  const c = data.check;
  return (
    <div style={{ marginTop: 10, borderTop: `1px dashed ${theme.line}`, paddingTop: 10 }}>
      <div style={{ fontWeight: 900, fontSize: 12.5, color: "#7B5EA7", marginBottom: 6 }}>🧪 سؤال المعلم للتأكد:</div>
      <div dir="ltr" style={{ textAlign: "left", fontWeight: 700, fontSize: 14.5, marginBottom: 8, lineHeight: 1.7 }}>{c.q}</div>
      {c.options.map((o, i) => {
        let st = {};
        if (picked !== null) { if (i === c.a) st = { borderColor: "#1F7A5C", background: "#1F7A5C22" }; else if (i === picked) st = { borderColor: "#B3402F", background: "#B3402F22" }; }
        return <button key={i} className="opt" style={{ ...st, fontSize: 13.5, padding: "10px 12px" }} onClick={() => {
          if (picked !== null) return;
          setPicked(i); play(i === c.a ? "correct" : "wrong");
          if (i === c.a && onBonus) onBonus(10);
        }}>{String.fromCharCode(65 + i)}. {o}</button>;
      })}
      {picked !== null && <div style={{ fontSize: 13, lineHeight: 1.8, background: picked === c.a ? "#1F7A5C14" : "#B3402F10", borderRadius: 10, padding: "8px 11px" }}>
        <b style={{ color: picked === c.a ? "#1F7A5C" : "#B3402F" }}>{picked === c.a ? "✓ رسخت الفكرة" : "لسا شوي —"}</b> {c.ex}
      </div>}
    </div>
  );
}
...
function Teacher({ g, theme, q, picked, fallbackCard, onClose, onBonus }) {
  const [tab, setTab] = useState("main");   // main | similar | ai
  ...
  const runAI = async () => {
    ...
    try {
      const j = await askAI(`أنت «المعلم» في لعبة تعليمية سعودية لاختبار القدرات (GAT English). مستوى الطالب في هذا الموضوع: «${level}». اشرح له هذا السؤال بأسلوب مختلف تمامًا وبسيط يناسب مستواه، بالعربية، مع مثال جديد محلول، ثم سؤال تحقق جديد.
السؤال: ${q.q}
الخيارات: ${(q.options || []).join(" | ")}
الإجابة الصحيحة: ${q.options ? q.options[q.a] : q.a}
${wrongOpt ? `الطالب اختار خطأً: ${wrongOpt}` : ""}
أجب فقط بـ JSON خام بلا أي أسوار: {"explain":"شرح عربي مبسط بأسلوب جديد (سطران إلى أربعة)","example":"مثال جديد محلول سطر أو سطران","check":{"q":"سؤال جديد بالإنجليزية بنفس المهارة","options":["أ","ب","ج","د"],"a":0,"ex":"شرح عربي قصير"}}`);
      setAi(j);
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 195, background: "rgba(0,0,0,.55)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: theme.bg, color: theme.text, width: "min(100%,620px)", maxHeight: "88vh", overflowY: "auto", borderRadius: "22px 22px 0 0", padding: "14px 16px 28px", animation: "drop .3s ease" }}>
        ...
        {tab === "main" && <>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {sq && <button className="btn" style={{ flex: 1, padding: 11, fontSize: 13 }} onClick={() => { play("click"); setTab("similar"); }}>🧪 جرّبني بسؤال مشابه</button>}
            <button className="btn gold" style={{ flex: 1, padding: 11, fontSize: 13 }} onClick={runAI}>🤖 اشرح بأسلوب آخر</button>
          </div>
        </>}
        ...
        {tab === "ai" && (
          <div className="card">
            {loading && <div style={{ textAlign: "center", padding: 20 }}><div style={{ fontSize: 30, animation: "pulse 1s infinite" }}>🤖</div><div style={{ fontSize: 13, color: theme.sub, marginTop: 6 }}>المعلم يعيد صياغة الشرح على مستواك ({level})…</div></div>}
            {!loading && ai?.err && <div style={{ fontSize: 13.5, textAlign: "center", color: theme.sub }}>المعلم الذكي (AI) يعمل داخل تطبيق Claude فقط — أما الشرح المتدرج والتلميحات والسؤال المشابه فتعمل دائمًا، حتى بلا إنترنت. <button className="btn ghost" style={{ marginTop: 8, padding: "8px 14px", fontSize: 12.5 }} onClick={runAI}>حاول مرة أخرى</button></div>}
            {!loading && ai && !ai.err && <>
              <div style={{ fontWeight: 900, fontSize: 12.5, color: "#C89235", marginBottom: 6 }}>🤖 بأسلوب مختلف (مضبوط على مستواك: {level}):</div>
              <div style={{ fontSize: 14, lineHeight: 2 }}>{ai.explain}</div>
              {ai.example && <div dir="auto" style={{ background: "#1F7A5C14", borderRadius: 10, padding: "9px 12px", marginTop: 8, fontSize: 13.5, lineHeight: 1.8 }}>📝 {ai.example}</div>}
              <AICheck theme={theme} data={ai} onBonus={onBonus} />
            </>}
            <button className="btn ghost" style={{ width: "100%", marginTop: 10, padding: 10, fontSize: 13 }} onClick={() => setTab("main")}>→ رجوع للشرح المتدرج</button>
          </div>
        )}
      </div>
    </div>
...
function AskTeacher({ g, theme, onBack, onBonus }) {
  ...
  const send = async () => {
    ...
    setLoading(true); setAns(null); play("click");
    const weak = SEC_AR[weakestOf(g, Object.keys(TOPIC_META))] || "عام";
    try {
      const j = await askAI(`أنت «المعلم» داخل أكاديمية لعبة تعليمية سعودية تحضّر لاختبار القدرات بالإنجليزية. أضعف مواضيع الطالب حاليًا: ${weak}. أجب عن سؤاله التالي بالعربية ببساطة وود، بمثال محلول، ثم سؤال تحقق قصير.
سؤال الطالب: «${txt.trim()}»
أجب فقط بـ JSON خام: {"explain":"إجابة عربية واضحة (٣-٦ أسطر)","example":"مثال محلول","check":{"q":"سؤال تحقق","options":["أ","ب","ج","د"],"a":0,"ex":"شرح قصير"}}`);
      setAns(j);
    } catch (e) { setAns({ err: true }); }
    setLoading(false);
  };
  return (
    <div style={{ animation: "drop .3s ease" }}>
      ...
      <div className="card" style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <TeacherFace size={46} />
        <div style={{ fontSize: 13.5, lineHeight: 1.8 }}><b>اسألني أي شيء عن الدروس</b> — كسور، نسب، تناظر، أزمنة… أشرحه على مستواك بمثال وسؤال تحقق.</div>
      </div>
      <div className="card">
        <textarea value={txt} onChange={e => setTxt(e.target.value)} rows={3} placeholder="مثال: ليش نقسم على القيمة القديمة في نسبة التغيّر؟"
          style={{ width: "100%", border: `1.5px solid ${theme.line}`, borderRadius: 12, padding: "10px 12px", fontFamily: "inherit", fontSize: 14, resize: "none", background: "transparent", color: theme.text }} />
        <button className="btn gold" style={{ width: "100%", marginTop: 8, padding: 12, opacity: txt.trim() && !loading ? 1 : .5 }} onClick={send}>{loading ? "المعلم يفكر…" : "📨 اسأل المعلم"}</button>
      </div>
      {ans?.err && <div className="card" style={{ textAlign: "center", fontSize: 13.5, color: theme.sub }}>سؤال المعلم الحر يحتاج تشغيل Arise داخل Claude — بقية الأكاديمية تعمل كاملة أوفلاين.</div>}
      {ans && !ans.err && (
        <div className="card">
          <div style={{ fontSize: 14, lineHeight: 2 }}>{ans.explain}</div>
          {ans.example && <div dir="auto" style={{ background: "#1F7A5C14", borderRadius: 10, padding: "9px 12px", marginTop: 8, fontSize: 13.5, lineHeight: 1.8 }}>📝 {ans.example}</div>}
          <AICheck theme={theme} data={ans} onBonus={onBonus} />
        </div>
      )}
    </div>
  );
...
function ReviewSession({ g, theme, onFinish, onExit }) {
  ...
  const [dIdx] = useState(() => items.map(it => Math.floor(Math.random() * it.u.drills.length)));
  const it = items[i];
  ...
  const drill = it ? it.u.drills[(dIdx[i] + (stage === "q2" ? 1 : 0)) % it.u.drills.length] : null;
...
function App() {
  ...
  const grant = (n, achId) => {
    if (achId && !n.ach.includes(achId)) {
      ...
      toast(`🏅 إنجاز: ${a.name}`); play("coin");
    }
  };

  const addRewards = (n, xp, coins) => {
    ...
    if (afterLvl > beforeLvl) {
      ...
      n.coins += bonus;
      toast(`⬆️ مستوى ${afterLvl}! +${bonus}🪙 +نقطة مهارة`);
      if (chest) setTimeout(() => toast("🎁 صندوق المستوى الكبير: +💡+🧊+🧪"), 600);
      play("levelup");
    }
  ...
  /* ---- الوقت والطاقة ---- */
  const spendSlot = (energyCost) => mut(n => {
  ...
  const onBattleEnd = (res) => {
    ...
    mut(n => {
      ...
      if (res.won && (n.mem.lost[bkey] || 0) > 0 && !n.mem.comeback[bkey]) {
        ...
        toast(`🔁 عودة الأبطال! الجميع سيتذكر هذا`);
      }
      ...
      addRewards(n, res.xpGained + (res.won ? res.xp : 0), res.won ? res.coins : 0);
      if (res.won) {
        n.stats.battles++;
        grant(n, "first");
        if (res.bestCombo >= 5) grant(n, "combo5");
        ...
        if (res.isGat) {
          ...
          if (n.gatScore >= 90) grant(n, "gat90");
          toast(`📜 درجة القدرات: ${n.gatScore}`);
        }
        ...
        if (res.isBoss) {
          ...
          if (res.chId === 1) grant(n, "ch1");
          if (res.chId >= n.chapter && res.chId < 7) {
            n.chapter = res.chId + 1;
            if (n.chapter === 4) { grant(n, "usa"); tl(n, "usa1", "🗽", "أول يوم في أمريكا", "برد ما تعرفه، لغة حولك من كل جهة، وسرير غريب… وحماس يمنعك من النوم."); }
          }
          if (res.chId === 6) grant(n, "grad");
          if (res.isFinal) {
            n.finalAcc = Math.round(res.acc * 100);
            const gs = n.gatScore || 0;
            n.ending = gs >= 90 && n.finalAcc >= 70 ? "legend" : (gs >= 80 || n.finalAcc >= 60) ? "good" : "open";
            if (n.ending === "legend") grant(n, "aramco");
  ...
  const doAct = (act, loc) => {
    ...
    if (act === "daily") {
      const today = new Date().toDateString();
      if (g.dailyDate === today) { toast("⏳ الصندوق يتجدد بكرة"); return; }
      const item = ["hint", "freeze", "potion"][Math.floor(Math.random() * 3)];
      mut(n => { n.dailyDate = today; n.coins += 40; n.items[item]++; addRewards(n, 10, 0); });
      play("coin");
      toast(`🎁 +40 🪙 + ${SHOP_ITEMS.find(s => s.id === item).icon} ${SHOP_ITEMS.find(s => s.id === item).name}`);
      return;
    }
  ...
  const claimTask = (scope, id) => mut(n => {
    ensurePeriods(n);
    ...
    st.claimed[id] = true;
    addRewards(n, d.r.xp || 0, d.r.coins || 0);
    if (d.r.item) n.items[d.r.item]++;
    n.season.pts += scope === "d" ? 15 : 40;
    toast(`✅ مهمة مكتملة: +${d.r.coins || 0}🪙${d.r.xp ? ` +${d.r.xp}XP` : ""}${d.r.item ? " +" + SHOP_ITEMS.find(s => s.id === d.r.item).icon : ""} • +${scope === "d" ? 15 : 40} نقطة موسم`);
    play("coin");
  });
  const claimSeason = (idx) => mut(n => {
    ensurePeriods(n);
      ...
      <div style={{ padding: "10px 14px 40px", maxWidth: 620, margin: "0 auto" }}>
        ...
        {view.s === "acad" && <Academy g={g} theme={theme} startReview={view.review}
          ...
          onReview={(records) => mut(n => {
            ...
            toast(`🧠 ثبّت ${kept} • أعاد للغد ${records.length - kept}`);
          })}
          ...
          onFinishUnit={(u, ph, opts = {}) => mut(n => {
            if (opts.bonusOnly) { addRewards(n, opts.bonusOnly, 0); return; }
            ...
            if (!review) {
              ...
              addRewards(n, R[0], R[1]);
              questEv(n, "correct", 3);
              tl(n, "firstlesson", "📖", "أول درس في الأكاديمية", "جلست، تعلمت مفهومًا، وطبقته بيدك. هكذا تُبنى العقول — درسًا فوق درس.");
              toast(`🎓 أتقنت: ${u.name} (+${R[0]} XP +${R[1]}🪙)`);
              if (phaseDone(n, ph.id)) { tl(n, "dip" + ph.id, ph.icon, `شهادة: ${ph.name.split(":")[1]}`, "مرحلة كاملة خلف ظهرك. المقتنيات 🎒 استقبلت شهادتك."); play("win"); setTimeout(() => toast(`${ph.icon} أكملت ${ph.name}!`), 700); }
            }
          })}
          onSimDone={(u, ph, res) => mut(n => {
            n.acad.units[u.id] = true;
            ...
            if (phaseDone(n, ph.id)) { tl(n, "dipP", ph.icon, `ختم الاحتراف`, "أنهيت آخر قاعة في الأكاديمية. من هنا وطالع… أنت المعلم."); play("win"); }
            toast(`🎭 درجتك التقديرية: ${res.score}`);
          })} />}

        {view.s === "choice" && <UniChoice g={g} theme={theme} onPick={(u) => {
          mut(n => { n.uni = u.name; n.done["3:choice"] = true; addRewards(n, 30, 20); tl(n, "uni", "✉️", `القبول: ${u.name}`, "قرأت خطاب القبول ثلاث مرات عشان تتأكد إنه اسمك فعلًا."); });
